...
def update_location():
    ...
    users_coll.update_one({"_id": ObjectId(session['user_id'])}, {"$set": {"lat": request.json['latitude'], "lon": request.json['longitude']}})
...
def book_ride():
    ...
    for d in drivers:
        if get_distance(user['lat'], user['lon'], d['lat'], d['lon']) <= 50.0:
            ...
            rides_coll.insert_one({"rider_id": session['user_id'], "driver_id": str(d['_id']), "otp": otp_code, "status": "pending"})
            return jsonify({"status": "success"})
